...
def aggregate_region_history_by_type():
    ...
    df2.to_csv("coalition_analytics_output/region_history_by_type.csv", index=False)
...
def aggregate_region_history_by_module():
    """
    Aggregate region history by module and calculate total value.
    """
    engine = DatabaseConfig("wcmkt2").engine
    with engine.connect() as conn:
        stmt = text("SELECT * FROM region_history")
    ...
    df2 = df2.merge(df4, left_on="type_id", right_on="typeID", how="left")
    df2 = df2[["typeID", "typeName", "groupID", "groupName", "categoryID", "categoryName", "average", "volume", "total_value"]]
    df2 = df2[df2["categoryID"] == 7]
    df2 = df2.sort_values(by="total_value", ascending=False)
    df2 = df2.reset_index(drop=True)

    df2.to_csv("coalition_analytics_output/region_history_by_module.csv", index=False)
...
def aggregate_region_history_by_ship():
    ...
    df2.to_csv("coalition_analytics_output/region_history_by_ship.csv", index=False)
...
if __name__ == "__main__":
    ...
    df2 = df2.sort_values(by="groupName", ascending=False)
    print(df2)
    df3 = aggregate_region_history_by_module()
    df3 = df3.sort_values(by="total_value", ascending=False)
    print(df3)